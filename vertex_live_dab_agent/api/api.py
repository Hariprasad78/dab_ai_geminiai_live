...
async def capture_select(request: CaptureSelectRequest) -> CaptureSourceResponse:
    ...
    if bool(status.get("hdmi_configured")):
        # Warm synchronously (bounded) so follow-up stream/test requests do not
        # race against lazy session probing right after a camera switch.
        with contextlib.suppress(Exception):
            await asyncio.wait_for(
                asyncio.to_thread(capture.ensure_hdmi_session, True),
                timeout=2.5,
            )
    return CaptureSourceResponse(**status)
